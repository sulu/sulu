...
test('Render in disabled state', () => {
    ...
    expect(screen.getByRole('group', {name: 'selection'})).toBeDisabled();
});
...
test('Trigger the change callback with undefined when the reset action is clicked', async() => {
    ...
    await user.click(screen.getByText('sulu_admin.please_choose'));
...
test('Updated data in EditOverlay should disappear when overlay is closed', async() => {
    ...
    await user.click(screen.getByLabelText('remove-1'));
    await user.click(screen.getByText('sulu_admin.add'));
    await user.type(getEditLineInputs()[1], 'Test3 Update');
