...
test('Call force delete when 410 media not found response and dialog is confirmed', (done) => {
    const deleteToolbarAction = createDeleteToolbarAction();
    deleteToolbarAction.resourceFormStore.resourceStore.id = 3;
    deleteToolbarAction.router.route.options.backView = 'sulu_test.list';
    ...
    setTimeout(() => {
        ...
        const mediaNotFoundDialog = element.find('Dialog[title="sulu_admin.force_delete_media_not_found_title"]');
        expect(mediaNotFoundDialog.exists()).toBe(true);
        expect(mediaNotFoundDialog.prop('open')).toEqual(true);

        const forceDeletePromise = Promise.resolve({});
        deleteToolbarAction.resourceFormStore.delete.mockReturnValueOnce(forceDeletePromise);

        mediaNotFoundDialog.find('Button[skin="primary"]').simulate('click');

        setTimeout(() => {
            expect(deleteToolbarAction.resourceFormStore.delete).toBeCalledWith({deleteLocale: false, force: true});
            expect(deleteToolbarAction.router.restore).toBeCalledWith('sulu_test.list', {locale: 'en'});
...
test('Do not call force delete when 410 media not found response and dialog is cancelled', (done) => {
    const deleteToolbarAction = createDeleteToolbarAction();
    deleteToolbarAction.resourceFormStore.resourceStore.id = 3;
    deleteToolbarAction.router.route.options.backView = 'sulu_test.list';
    ...
    setTimeout(() => {
        ...
        // Click cancel
        mediaNotFoundDialog.find('Button[skin="secondary"]').simulate('click');
        ...
        setTimeout(() => {
            ...
            expect(deleteToolbarAction.resourceFormStore.delete).toBeCalledTimes(1);
            expect(
                element.find('Dialog[title="sulu_admin.force_delete_media_not_found_title"]').exists()
            ).toBe(false);
