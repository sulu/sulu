...
class CacheClearCommand extends BaseCacheClearCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $io->comment(sprintf('Clearing the <info>App cache</info> for the <info>%s</info> environment with debug <info>%s</info>',
            $kernel->getEnvironment(), var_export($kernel->isDebug(), true)));

        parent::execute($input, $nullOutput);

        $io->success(sprintf('App cache for the "%s" environment (debug=%s) was successfully cleared.',
            $kernel->getEnvironment(), var_export($kernel->isDebug(), true)));
    }
}
