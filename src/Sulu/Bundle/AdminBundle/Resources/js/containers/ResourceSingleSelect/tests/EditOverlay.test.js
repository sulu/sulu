...
test('Should only delete items from  ResourceStoreList if data is only deleted', async() => {
    ...
    await user.click(screen.getByLabelText('remove-0'));
...
test('Should update ResourceStoreList if data is changed and confirm button is clicked', async() => {
    ...
    await user.type(getEditLineInputs()[2], 'Test 3');
    await user.type(getEditLineInputs()[3], 'Test 4');
...
test('Adding the same field as already existing should not add it', async() => {
    ...
    await user.click(screen.getByText('sulu_admin.add'));
    await user.type(getEditLineInputs()[2], 'Test 2');
    await user.click(screen.getByLabelText('remove-0'));
    await user.click(screen.getByTestId('confirm'));
...
test('Adding the same field twice should add it only once', async() => {
    ...
    await user.type(getEditLineInputs()[2], 'Test 3');
    await user.type(getEditLineInputs()[3], 'Test 3');
