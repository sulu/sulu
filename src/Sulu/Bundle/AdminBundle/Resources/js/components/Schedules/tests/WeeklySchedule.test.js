...
test('Change start date', () => {
    ...
    const weeklySchedule = shallow(
        <WeeklySchedule index={0} onChange={changeSpy} value={{end: '12:00:00', type: 'weekly'}} />
    );

    weeklySchedule.find('Field[label="sulu_admin.start"] DatePicker').simulate('change', date);
...
test('Change end date', () => {
    ...
    weeklySchedule.find('Field[label="sulu_admin.end"] DatePicker').simulate('change', date);
...
test('Change selected days', () => {
    ...
    weeklySchedule.find('Field[label="sulu_admin.weekdays"] MultiSelect').simulate('change', ['wednesday', 'thursday']);
