...
The `totp` and `google` two factor methods are now available in the admin profile when the
`scheb/2fa-totp` respectively `scheb/2fa-google-authenticator` package is installed and enabled in
the `scheb_two_factor` configuration. Both work with all common authenticator apps
(Google Authenticator, Microsoft Authenticator, ...) and use the same guided setup: scanning a QR
code and confirming it with a generated code. Both methods are TOTP based, so installing one of the
two packages is sufficient - when both are installed, both appear in the method selection.
