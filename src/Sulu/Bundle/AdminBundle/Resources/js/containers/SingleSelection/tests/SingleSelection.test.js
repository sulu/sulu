...
test('Should open and close an overlay', async() => {
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', {name: 'selection overlay'}))
...
test(
    ...
    async() => {
        ...
        expect(screen.getByLabelText('item')).toBeEnabled();
        ...
        await waitFor(() => expect(screen.getByLabelText('item')).toBeDisabled());
    }
);
