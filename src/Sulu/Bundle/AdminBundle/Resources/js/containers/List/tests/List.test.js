...
jest.mock('../../../components/Dialog', () => {
    ...
    return jest.fn((props) => props.open
...
jest.mock('../../SingleListOverlay', () => {
    ...
    return jest.fn((props) => {
        ...
        mockSingleListOverlayProps[overlayType] = props;
...
async function clickOpenDialogButton(title: string, name: string) {
    ...
    await user.click(within(getOpenDialog(title)).getByRole('button', {name}));
}
...
test('Pass correct options and metadataOptions to SingleListOverlays', () => {
    ...
    expect(mockSingleListOverlayProps.move).toEqual(expect.objectContaining({
        metadataOptions: {id: 1},
        reloadOnOpen: true,
    }));
    expect(mockSingleListOverlayProps.copy).toEqual(expect.objectContaining({
        metadataOptions: {id: 1},
        reloadOnOpen: true,
    }));
});
...
test('SingleListOverlay should disappear when onRequestItemCopy callback is called and overlay is closed', async() => {
    ...
    const copyOverlay = screen.getByRole('dialog', {name: 'copy overlay'});
    expect(copyOverlay).toHaveAttribute('data-clear-selection-on-close', 'true');
    expect(copyOverlay).not.toHaveAttribute('data-disabled-ids');
    expect(copyOverlay).toHaveAttribute('data-resource-key', 'test');
    expect(copyOverlay).toHaveAttribute('data-list-key', 'test_list');

    await user.click(screen.getByRole('button', {name: 'close-copy'}));

    await waitFor(() => expect(screen.queryByRole('dialog', {name: 'copy overlay'})).not.toBeInTheDocument());
...
test('ListStore should copy item when onRequestItemCopy callback is called and overlay is confirmed', async() => {
    ...
    await user.click(screen.getByRole('button', {name: 'request-copy'}));
    expect(screen.getByRole('dialog', {name: 'copy overlay'})).toBeInTheDocument();
...
test('Should close the copy overlay and call onCopyError when copying fails', async() => {
    const user = userEvent.setup();
    const errorData = {detail: 'Copying is not allowed'};
    const copyResponse = {json: jest.fn().mockResolvedValue(errorData)};
    const copyErrorSpy = jest.fn();
    const listStore = createListStore();
    listStore.copy.mockRejectedValue(copyResponse);

    render(<List adapters={['test']} onCopyError={copyErrorSpy} store={listStore} />);
    ...
    await waitFor(() => expect(copyErrorSpy).toHaveBeenCalledWith(errorData));
    expect(screen.queryByRole('dialog', {name: 'copy overlay'})).not.toBeInTheDocument();
});
...
test('SingleListOverlay should disappear when onRequestItemMove callback is called and overlay is closed', async() => {
    ...
    const moveOverlay = screen.getByRole('dialog', {name: 'move overlay'});
    expect(moveOverlay).toHaveAttribute('data-disabled-ids', JSON.stringify([5]));
    expect(moveOverlay).toHaveAttribute('data-resource-key', 'test');
    expect(moveOverlay).toHaveAttribute('data-list-key', 'test_list');

    await user.click(screen.getByRole('button', {name: 'close-move'}));

    await waitFor(() => expect(screen.queryByRole('dialog', {name: 'move overlay'})).not.toBeInTheDocument());
...
test('ListStore should not move when onRequestItemMove callback is called and permission dialog is denied', async() => {
    ...
    expect(screen.getByRole('dialog', {name: 'move overlay'})).toBeInTheDocument();
});
