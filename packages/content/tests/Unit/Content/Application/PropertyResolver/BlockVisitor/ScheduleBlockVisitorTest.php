...
class ScheduleBlockVisitorTest extends TestCase
{
    ...
    public function testShouldNotSkipWithObjectAsSettings(): void
    {
        ...
        $this->assertEquals($block, $this->scheduleBlockVisitor->visit($block));
    }
    ...
    public static function provideVisit(): array
    {
        return [
            [
                [
    ...
    /**
     * @param string[] $requestCacheLifetimes
     */
    #[\PHPUnit\Framework\Attributes\DataProvider('provideVisit')]
    public function testVisit(array $settings, string $now, bool $skip, array $requestCacheLifetimes): void
    {
        $nowDateTime = new \DateTime($now);
        $this->requestAnalyzer->getDateTime()->willReturn($nowDateTime);

        $block = ['name' => 'type1', 'metadata' => new Metadata([]), 'settings' => $settings];

        if (false === $skip) {
            $this->assertEquals($block, $this->scheduleBlockVisitor->visit($block));
        } else {
            $this->assertNull($this->scheduleBlockVisitor->visit($block));
        }
