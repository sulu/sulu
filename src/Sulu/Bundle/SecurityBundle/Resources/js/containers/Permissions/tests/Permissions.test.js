...
const renderPermissions = (customProps: Object) => {
    ...
    const view = render(<Permissions {...props} ref={permissionsRef} />);

    return {
        ...view,
        getInstance: () => {
            if (!permissionsRef.current) {
                throw new Error('Expected Permissions instance ref to be available');
            }
        ...
        setProps: (newProps: Object) => {
            ...
            view.rerender(<Permissions {...props} ref={permissionsRef} />);
        },
    };
};
...
test('Dispose autorun on unmount', () => {
    ...
    permissions.getInstance().systemDisposer = systemDisposerSpy;
    permissions.unmount();
