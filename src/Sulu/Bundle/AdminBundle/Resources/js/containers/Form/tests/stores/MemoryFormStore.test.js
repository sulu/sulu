...
test('Validate should merge the errors of a field validator under its data path', () => {
    ...
    expect(memoryFormStore.validate()).toEqual(true);
    expect(memoryFormStore.errors).toEqual({});
});

test('Validate should keep the schema errors next to the field validator errors', () => {
    const memoryFormStore = new MemoryFormStore({attributes: {}}, {}, {type: 'object', required: ['title']});
...
test('A removed field validator should not run any more', () => {
    const memoryFormStore = new MemoryFormStore({attributes: {}}, {}, {type: 'object'});
    const validator = jest.fn(() => ({'7': {keyword: 'required', parameters: {}}}));
    const removeValidator = memoryFormStore.addFieldValidator('/attributes', validator);
