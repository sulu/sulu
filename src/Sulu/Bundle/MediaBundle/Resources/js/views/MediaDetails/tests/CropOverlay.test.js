...
test('Do not save if undefined is passed', () => {
    const closeSpy = jest.fn();
    ...
    cropOverlay.instance().mediaFormatStore.getFormatOptions.mockReturnValue(undefined);
    ...
    return formatsPromise.then(() => {
        ...
        expect(cropOverlay.instance().mediaFormatStore.updateFormatOptions).not.toBeCalled();
        expect(closeSpy).toBeCalledWith();
    });
});
