...
test('Call onChange prop when something changed', async() => {
    ...
    await editorPromise;
    getMockCallArg(editor.model.document.on, 0, 1)();
...
test('Call onBlur prop when CKEditor5 fires its blur event', async() => {
    ...
    await editorPromise;
    getMockCallArg(editor.editing.view.document.on, 0, 1)();
...
test('Call onFocus prop when CKEditor5 fires its focus event', async() => {
    ...
    const {asFragment} = render(<CKEditor5 onChange={jest.fn()} onFocus={focusSpy} value={undefined} />);
