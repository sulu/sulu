...
```sql
ALTER TABLE me_file_version_meta ADD ai_disclosure_text LONGTEXT DEFAULT NULL;
ALTER TABLE me_file_versions ADD origin VARCHAR(191) DEFAULT NULL, ADD aiDisclosureDisabled TINYINT(1) DEFAULT NULL, ADD aiDisclosureIconVariant VARCHAR(191) DEFAULT NULL;
```
...
package is installed and enabled in the `scheb_two_factor` configuration. It works with all common
authenticator apps (Google Authenticator, Microsoft Authenticator, ...). Setting up the method
requires scanning a QR code and confirming it with a generated code. The scheb `google` method is
not offered, because it is TOTP based as well and would be a redundant choice.

When `scheb/2fa-backup-code` is installed and enabled, backup codes can be generated for every
active two factor method. Backup codes generated by Sulu are stored as password hashes and are only
shown once on generation. Existing plain text backup codes stay valid.
