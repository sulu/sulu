...
define([
    ...
    /**
     * Changes the area selection to the selected format.
     *
     * @param formatId
     */
    var changeFormat = function(formatId) {
            this.selectedFormat = this.formats[formatId];
            this.saved = true;
         ...
         * Asks for confirmation if the current crop has not been saved.
         *
         * @param {Object} dropdownItem The clicked dropdown item
         */
        formatsDropdownHandler = function(dropdownItem) {
            if (dropdownItem.id === this.selectedFormat.key) {
                return;
            }

            if (!this.saved) {
                this.sandbox.sulu.showConfirmationDialog({
                    title: 'sulu-media.unsaved-crop-title',
                    description: 'sulu-media.unsaved-crop-description',
                    callback: function(confirmed) {
                        if (!confirmed) {
                            this.sandbox.emit(
                                'husky.toolbar.cropping-' + this.media.id + '.item.change',
                                'formats',
                                this.selectedFormat.key
                            );
                            this.sandbox.emit(
                                'husky.toolbar.cropping-' + this.media.id + '.item.unmark',
                                this.selectedFormat.key
                            );

                            return;
                        }
                        changeFormat.call(this, dropdownItem.id);
                    }.bind(this)
                });
            } else {
                changeFormat.call(this, dropdownItem.id);
