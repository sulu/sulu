    ...
    return {
        ...
        reload: function(options) {
            ...
            if (!!options.preselected) {
                this.options.preselected = (!!options.preselected) ? options.preselected : this.options.preselected;
            }
            this.setVariables();
            this.render();
        },
        ...
        startGroupDatagrid: function(id, $container) {
            ...
            this.bindDatagridEvents(instanceName, id);
        ...
        /**
         * Binds the custom events for a datagrid
         * @param instanceName {string} the instance-name of the datagrid
         * @param groupId {Number|String} the id of the group
         */
        bindDatagridEvents: function(instanceName, groupId) {
            if (this.dataGridEventsBound.indexOf(instanceName) === -1) {

                this.dataGridEventsBound.push(instanceName);

                // store number of selected elements
                this.sandbox.on('husky.datagrid.' + instanceName + '.number.selections', function(number) {
                    this.group[groupId].selectedElements = number;
                    this.emitElementsSelected();
                }.bind(this));

                // edit single record on datagrid click
                this.sandbox.on('husky.datagrid.' + instanceName + '.item.click', function(recordId) {
                    this.lastClickedGrid = this.group[groupId].datagridName;
                    this.sandbox.emit(RECORD_CLICKED.call(this), recordId);
                }.bind(this));

                // throw height changed event after initialization has finished
                this.sandbox.on('husky.datagrid.' + instanceName + '.view.rendered', function() {
