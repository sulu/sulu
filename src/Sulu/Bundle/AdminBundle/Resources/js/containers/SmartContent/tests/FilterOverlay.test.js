...
jest.mock('../../../containers/MultiListOverlay', () => jest.fn((props) => {
    ...
    return mockReact.createElement(
        ...
        props.open && mockReact.createElement(
            'div',
            {
                'aria-label': props.resourceKey + ' selection',
                role: 'dialog',
            },
            mockReact.createElement(
                'button',
...
test('Do not display if open is set to false', () => {
    ...
    expect(screen.queryByRole('dialog', {name: 'filter overlay'})).not.toBeInTheDocument();
});
...
test('Fill all fields using and update SmartContentStore on confirm', async() => {
    ...
    expect(screen.getByRole('dialog', {name: 'pages selection'})).toBeInTheDocument();
    await user.click(screen.getByLabelText('confirm-pages'));
    expect(screen.queryByRole('dialog', {name: 'pages selection'})).not.toBeInTheDocument();
    ...
    expect(screen.getByRole('dialog', {name: 'categories selection'})).toBeInTheDocument();
    await user.click(screen.getByLabelText('confirm-categories'));
    expect(screen.queryByRole('dialog', {name: 'categories selection'})).not.toBeInTheDocument();
