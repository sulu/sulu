 ...
 */
class ExcerptFormPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($finder as $file) {
            ...
            foreach ($tagNodes as $tagNode) {
                ...
                if ('sulu_content.content_excerpt_form' !== $tagNode->getAttribute('name')) {
                    continue;
                }

                if (empty($instanceOf)) {
                    throw new \RuntimeException(
                        'Tags with the name "sulu_content.content_excerpt_form" must have a valid "instanceOf" attribute! '
                    );
                }
        ...
        \uasort($excerptForms, static function($a, $b) {
            return $b['priority'] <=> $a['priority'];
        });

        $container->setParameter('sulu_content.content_excerpt_forms', $excerptForms);
    }
}
