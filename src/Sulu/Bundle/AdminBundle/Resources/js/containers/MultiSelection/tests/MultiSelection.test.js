...
jest.mock('../../../components/MultiItemSelection', () => {
    const MultiItemSelectionMock: any = jest.fn((props) => {
        ...
        return mockReact.createElement(
            ...
            props.sortable && mockReact.createElement(
                'button',
                {
                    'aria-label': 'sort',
...
test('Show with passed icon and label and open overlay', async() => {
    ...
    await user.click(screen.getByLabelText('left-button'));
    ...
    expect(screen.getByRole('dialog', {name: 'selection overlay'})).toBeInTheDocument();
});
...
test('Pass correct props to MultiItemSelection component', () => {
    ...
    expect(screen.getByLabelText('left-button')).toBeDisabled();
...
test('Render with disabled item', () => {
    ...
    expect(screen.getByLabelText('item-1')).toBeEnabled();
    expect(screen.getByLabelText('item-2')).toBeDisabled();
});
...
test('Update requestParameters with the display properties added to the fields request parameter', async() => {
    const locale = observable.box('en');
    ...
    await waitFor(() => expect((getSelectionStore(): any).setRequestParameters)
...
test('Should not open an overlay on icon-click when disabled', async() => {
    ...
    expect(screen.queryByRole('dialog', {name: 'selection overlay'})).not.toBeInTheDocument();
});
...
test('Should close an overlay using the close button', async() => {
    ...
    await user.click(screen.getByLabelText('left-button'));
    expect(await screen.findByRole('dialog', {name: 'selection overlay'})).toBeInTheDocument();
...
test('Should remove an item when the remove button is clicked', async() => {
    ...
    await user.click(screen.getByLabelText('remove-7'));
