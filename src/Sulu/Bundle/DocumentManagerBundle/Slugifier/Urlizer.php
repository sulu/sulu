...
namespace Sulu\Bundle\DocumentManagerBundle\Slugifier;

/**
 * @internal use NodeNameSlugifier service "sulu_document_manager.node_name_slugifier" instead.
 *
 * This is a copy of the Behat/Transliterator
 * https://github.com/Behat/Transliterator/blob/v1.2.0/src/Behat/Transliterator/Transliterator.php
 * which is a newer version of the Ferrandini/Urlizer
 * https://github.com/aferrandini/Urlizer/blob/1.0.0/Urlizer.php
 *
    ...
    public static function seemsUtf8($string)
    {
        ...
        for ($i = 0; $i < $stringLength; ++$i) {
            if (\ord($string[$i]) < 0x80) { // 0bbbbbbb
                continue;
            } elseif (0xC0 == (\ord($string[$i]) & 0xE0)) { // 110bbbbb
                $n = 1;
            } elseif (0xE0 == (\ord($string[$i]) & 0xF0)) { //1110bbbb
                $n = 2;
            } elseif (0xF0 == (\ord($string[$i]) & 0xF8)) { // 11110bbb
                $n = 3;
            } elseif (0xF8 == (\ord($string[$i]) & 0xFC)) { // 111110bb
                $n = 4;
            } elseif (0xFC == (\ord($string[$i]) & 0xFE)) { // 1111110b
                $n = 5;
            } else {
            ...
            for ($j = 0; $j < $n; ++$j) { // n bytes matching 10bbbbbb follow ?
                if (++$i === $stringLength || (0x80 !== (\ord($string[$i]) & 0xC0))) {
                    return false;
                }
            }
        }
