...
test('Should destroy create new formstore when block settings overlay is opened for another block', () => {
    ...
    act(() => {
        ...
        (BlockCollection: any).mock.calls.at(-1)[0].onSettingsClick(1);
    });

    expect(screen.getByRole('heading', {name: 'sulu_admin.block_settings'})).toBeInTheDocument();
    const secondFormStore = (FormOverlay: any).mock.calls.at(-1)[0].formStore;
...
test('Should not close block settings overlay when confirm button is clicked with invalid data', async() => {
    ...
    const formStore = (FormOverlay: any).mock.calls.at(-1)[0].formStore;
...
test('Should display and update correct icons based on block settings data and schema', async() => {
    ...
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[0]).toEqual(['su-hide']);
    ...
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[0]).toEqual([]);
});
...
test('Should recalculate visibleCondition only of changed blocks', async() => {
    ...
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[0]).toEqual(['su-hide']);
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[1]).toEqual(['su-hide']);
    ...
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[0]).toEqual(['su-hide']);
    expect((BlockCollection: any).mock.calls.at(-1)[0].icons[1]).toEqual([]);
});
