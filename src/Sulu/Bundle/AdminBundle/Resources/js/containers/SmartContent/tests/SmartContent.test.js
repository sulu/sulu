...
jest.mock('../FilterOverlay', () => jest.fn((props) => {
    ...
    return mockReact.createElement(
        'div',
        {
            'aria-label': 'smart content filters',
            role: 'dialog',
        },
...
test('Open and closes the FilterOverlay when the icon is clicked', async() => {
    ...
    expect(screen.queryByRole('dialog', {name: 'smart content filters'})).not.toBeInTheDocument();

    await user.click(getButtonByIcon('su-filter'));
    expect(screen.getByRole('dialog', {name: 'smart content filters'})).toBeInTheDocument();
