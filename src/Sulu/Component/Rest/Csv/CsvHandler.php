...
class CsvHandler
{
    ...
    public function createResponse(ViewHandlerInterface $handler, View $view, Request $request, $format)
    {
        ...
        $config->setDelimiter($this->convertValue($request->query->getString('delimiter', ';'), self::$delimiterMap));
        $config->setNewline($this->convertValue($request->query->getString('newLine', '\\n'), self::$newLineMap));
        $config->setEnclosure($request->query->getString('enclosure', '"'));
        $config->setEscape($request->query->getString('escape', '\\'));
