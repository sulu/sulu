...
jest.mock('../../../components/Overlay', () => jest.fn((props) => (
    props.open
        ? mockReact.createElement(
            'section',
            {
                'aria-label': 'resource locator history',
                role: 'dialog',
            },
...
async function openHistoryOverlay() {
    ...
    await user.click(screen.getByText('sulu_admin.show_history'));
}
...
test('Close overlay if button is clicked', async() => {
    ...
    expect(screen.queryByRole('dialog', {name: 'resource locator history'})).not.toBeInTheDocument();

    await openHistoryOverlay();
    expect(screen.getByRole('dialog', {name: 'resource locator history'})).toBeInTheDocument();
...
test('Do not delete if confirmation dialog is cancelled', async() => {
    ...
    await user.click(await screen.findByLabelText('su-trash-alt-3'));

    expect(screen.getByTestId('dialog')).toBeInTheDocument();

    await user.click(screen.getByLabelText('dialog-cancel'));
...
test('Delete if confirmation dialog is confirmed', async() => {
    ...
    await user.click(screen.getByLabelText('dialog-confirm'));
